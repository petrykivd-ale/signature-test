DEFAULT_SIGNATURE_PROMPT = (
    "Create a cool looking clean signature on one line with dark blue on paper for the name Allon Mason top down view straight view. "
...
CHAOTIC_SIGNATURE_PROMPT = (
    "Create a chaotic, fast-motion signature with dark blue ink for the name Allon Mason, "
    "featuring sharp angles, rapid loops, and an aggressive underline, top-down view on white paper."
)

ELEGANT_SIGNATURE_PROMPT = (
    "Create a graceful, calligraphic signature with dark blue ink for Allon Mason, featuring long flourishes, "
    "thin upstrokes, and thick downstrokes, inspired by fountain pen calligraphy, top-down view."
)

FEMININE_HEART_SIGNATURE_PROMPT = (
    "Generate a rounded, bubbly, playful signature for Allon Mason using dark blue ink, with wide loops and heart-shaped flourishes, "
    "seen from a top-down perspective on clean white paper."
)

FEMININE_STAR_SIGNATURE_PROMPT = (
    "Generate a rounded, bubbly, playful signature for Allon Mason in dark blue ink, "
    "with wide loops and delicate star-shaped flourishes integrated into the strokes, "
    "seen from a top-down perspective on clean white paper."
)

FEMININE_GEOMETRY_PROMPT = (
    "Create a soft, whimsical signature for Allon Mason using dark blue ink, "
...
CORPORATE_SIGNATURE_PROMPT = (
    "Create a neat, minimal, corporate-style signature for Allon Mason with evenly spaced block cursive, "
    "light blue ink on white paper, seen top-down, with clean pen strokes and no flourish."
)

OLD_HOLLYWOOD_SIGNATURE_PROMPT = (
    "Generate a dramatic, vintage-style signature for Allon Mason in dark blue ink, "
...
GRAFFITI_SIGNATURE_PROMPT = (
    "Create a graffiti-style signature for Allon Mason in dark blue ink, with exaggerated motion, "
    "disconnected letters, abstract strokes, and street-art energy, top-down on paper."
)

MIN_SHARP_SIGNATURE_PROMPT = (
    "Generate a signature for Allon Mason using sharp, minimal, geometric strokes with dark blue ink, "
    "highly angular and simplified, almost cryptic in style, seen from a top-down view."
)

CHILDLIKE_SIGNATURE_PROMPT = (
    "Create a childlike, uneven signature for Allon Mason with dark blue ink, featuring shaky lines, "
    "inconsistent spacing, and innocent loops, as if drawn by a young person, top-down view."
)

FLAMBOYANT_SIGNATURE_PROMPT = (
    "Generate an artistic, dramatic signature for Allon Mason with bold, looping strokes, exaggerated tails, "
    "and expressive swirls in dark blue ink, top-down view on textured paper."
)

MASCULINE_SIGNATURE_PROMPT = (
    "A bold, masculine signature of the name Allon Mason in dark blue ink, featuring strong, "
...
FLAMBOYANT_FEMININE_PROMPT = (
    "A highly stylized, flowing signature for the name Allon Mason, written in dark blue ink from a top-down perspective. "
    "The signature features exaggerated loops, elegant curves, and dramatic flourishes, "
...
EVALUATION_PROMPT = (
    "Evaluate this signature image for the name Allon Mason. Does it look professional, clear, and readable? "
    "Ensure the signature represents the name accurately without missing or extra letters. "
    "It should have smooth, continuous lines and avoid unnecessary flourishes. Your answer should be YES or NO."
)
